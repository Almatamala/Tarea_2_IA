...
int action_sel=1; // 1 is greedy, 2 is e-greedy
...
int num_episodes=100; //total learning episodes
...
{ // Based on the action selection method chosen, it selects an action to execute next
    int greed=0; 
    float max = Qvalues[x_pos][y_pos][0];
    for(int i=0;i<4;i++){
        if(max < Qvalues[x_pos][y_pos][i]){
            max = Qvalues[x_pos][y_pos][i];
            greed=i;
        }
    }
    action_taken=greed;
    ...
    {
        return greed; //Currently returing a random action, need to code the greedy strategy
    }
    ...
    {
        if(rand()/RAND_MAX>0.05){//Implemente la estrategia (policy) epsilon-greedy para la selección de acciones (con un valor de epsilon = 0.05) 
            return greed;
        }else return rand()%4; //Currently returing a random action, need to code the e-greedy strategy
...
void move(int action)
{
    ...
    {
    //Implemente acciones estocásticas (que sólo un 80% de las veces el agente se mueva donde se le indica, el 10% de las veces 
    //se mueve a la derecha de la dirección deseada y el otro 10% de las veces se mueve a la izquierda de la dirección deseada).
        int random = rand()%100;
        if(random>80){
            if(random<90){
                action = 1;
...
void update_q_prev_state() //Updates the Q value of the previous state
{
    ...
    if(!( ((x_pos==goalx)&&(y_pos==goaly)) ||((environment==1)&&(x_pos==goalx)&&(y_pos==(goaly-1)))||((environment==2)&&(x_pos>0)&&(x_pos<goalx)&&(y_pos==0))) )
    {
        ...
        Qvalues[prev_x_pos][prev_y_pos][action_taken]= (1-learn_rate)*Qvalues[prev_x_pos][prev_y_pos][action_taken] + (learn_rate*(reward[x_pos][y_pos] + (disc_factor*Qvalues[x_pos][y_pos][action_taken]))); //How should the Q values be updated?
    }
...
int main(int argc, char* argv[])
{
    ...
    for(i=0;i<num_episodes;i++)
    {
        ...
        file <<current_episode<<", "<<cum_reward<<endl;// Imprimir en un archivo los pares [episodio, reward acumulado]
